...
def _translate_keywords(words_to_translate: list[str], from_language: str) -> list[str]:
    ...
    try:
        ...
        result = argostranslate.translate.translate(
            text_to_translate, from_code, to_code
        )
        if result:
            return [w.strip().lower() for w in result.split("|")]
    except Exception as e:
        print(
            f"Warning: Failed to translate keywords from {from_language} using Argos: {e}"
        )
...
def extract_keywords_from_text(text: str, language: str) -> Set[str]:
    ...
    return {stemmer(w) for w in raw_words if len(w) >= 3 and w not in stop_words}


@disk_cache
def summarize(text: str, language: str, length: str = "medium") -> str:
    if not text.strip():
        return ""
    ...
    # Smart summarization: adjust based on requested length
    length_map = {
        "short": max(1, min(2, total_sentences // 2)),
        "medium": max(2, min(4, total_sentences // 2)),
        "long": max(3, min(6, total_sentences // 2)),
    }
...
@disk_cache
def get_category(
    en_keywords: Set[str], en_title_keywords: Set[str] | None = None
) -> str:
    en_stemmer = Stemmer("english")
    title_keys = en_title_keywords or set()
    ...
    for cat, keywords in ENGLISH_CATEGORY_KEYWORDS.items():
        ...
        # Scoring: Title matches are worth 3x more than content matches
        content_score = len(en_keywords.intersection(stemmed_cat_keywords))
        title_score = len(title_keys.intersection(stemmed_cat_keywords)) * 3.0
